...
const corrections = [
  { id: 258, keyword: 'Fraude em Licitações via IA em Fintechs', md: md258 },
  { id: 259, keyword: 'Zero-Day AI Vulnerabilities and how to solve it in practice under Ley 1581', md: md259 },
  { id: 260, keyword: 'demostrar cumplimiento con Ley 25.326 + Ley 27.741', md: md260 },
  { id: 261, keyword: 'TCO de remediar Fraude em Licitações via IA', md: md261 },
  { id: 262, keyword: 'IA on-premise soberana para Gobierno / Salud (Chile)', md: md262 }
];

let count = 0;
data.forEach(s => {
  const match = corrections.find(c => s.title && s.title.includes(c.keyword));
  if (match) {
    s.contentMarkdown = match.md;
    s.status = 'ready';
    count++;
    console.log('[-] Artigo ' + match.id + ' marcado para correção cirúrgica.');
  }
});

if (count === 5) {
  fs.writeFileSync(file, JSON.stringify(data, null, 2));
  console.log('✅ CORREÇÃO CIRÚRGICA CONCLUÍDA: 5 artigos atualizados com Role Bleeding eliminado e Footer injetado.');
} else {
  console.log('⚠️ ALERTA: Apenas ' + count + ' artigos encontrados.');
}
